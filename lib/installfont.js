...
module.exports = function (fontPath, callback, options){
    ...
    if(stats.isDirectory()){
        installFontsByDirectory(fontPath);
    } else {
        installFontFile(fontPath);
    }
};
...
function installFontsByDirectory(fontPathDir){
    ...
    fs.exists(fontPathDir, function (exists) {
        if (exists) {
            getFontFiles(fontPathDir);
        } else {
            invokeCallBack(new Error("Specified directory does not exist: " + fontPathDir));
        }
    });
}
...
function installFontFile(fontFilePath){
    ...
    fs.exists(fontFilePath, function (exists) {
        if (exists) {
            checkInstalledFontsForFile(fontFilePath);
        } else {
            invokeCallBack(new Error("Specified font file does not exist: " + fontFilePath));
        }
    });
}
...
function checkInstalledFontsForFile(fontFilePath) {
    fs.readdir('c:/Windows/Fonts', function (err, files) {
        ...
        copyFontFile(fontPathDir, fontName, isInstalled);
