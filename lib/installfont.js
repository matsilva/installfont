...
module.exports = function (fontPath, callback, options) {
  if(typeof callback === 'function') invokeCallBack = _invokeCallBack.bind(this, callback);
...
function installFontsByDirectory(fontPathDir) {
  ...
  fs.exists(fontPathDir, function (exists) {
    if (exists) {
        console.log('66 fontPathDir exists');
...
function checkInstalledFonts(fontPathDir, fontName) {
    ...
    if(platform.toLowerCase().indexOf('linux') != -1) {
       ...
       if (!_sysFontDir) {
          ...
          return console.log('Could not associate font with system folder: ', fontName);
       }
    }
...
function checkInstalledFontsForFile(fontFilePath) {
    ...
    if(platform.toLowerCase().indexOf('linux') != -1) {
        ...
        console.log("sysfont dir: ", _sysFontDir);
       if (!_sysFontDir) {
...
function tryInstallFonts(fontPathDir) {
    console.log('fontCount: ', fontCount, 'currentFontcount: ', currentFontCount);
