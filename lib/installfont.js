...
module.exports = function (fontPath, callback, options) {
    ...
    if(options){
        ...
        if (options.installForWindowsUser === true && platform.toLowerCase().indexOf('win32') !== -1) {

            sysFontDir = path.join(process.env.LOCALAPPDATA,'Microsoft','Windows','Fonts');
        }
    }
...
function checkInstalledFonts(fontPathDir, fontName) {
    ...
    fs.readdir(_sysFontDir, function (err, files) {
        if (err) { console.log(`fs error ${err} at ${sysFontDir}`); } else {
